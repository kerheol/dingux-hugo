...
# define MENU_LOAD_SLOT   MENU_VOLUME+1
#else
...
# define MENU_LOAD_CD     MENU_LOAD_ROM+1

# define MENU_EDITOR      MENU_LOAD_CD+1
# define MENU_CHEATS      MENU_EDITOR+1
# define MENU_KEYBOARD    MENU_CHEATS+1
# define MENU_JOYSTICK    MENU_KEYBOARD+1
# define MENU_SETTINGS    MENU_JOYSTICK+1
...
psp_menu_display_save_name()
{
  ...
  psp_sdl_back2_print(280 - (6*Length),  5, buffer, PSP_MENU_TEXT2_COLOR);
}
...
# endif
  ...
  if (HUGO.hugo_save_state[cur_slot].thumb) {
    psp_sdl_blit_thumb(160,125, HUGO.hugo_save_state[cur_slot].surface);
  } else {
    psp_sdl_blit_thumb(160,125, thumb_surface);
  }
...
psp_main_menu_reset(void)
{
  ...
  psp_sdl_back2_print(130, 110, "Reset Hu-Go !", PSP_MENU_WARNING_COLOR);
  psp_sdl_flip();
  hugo_emulator_reset();
  sleep(1);
}
...
psp_main_menu_load(int format)
{
  ...
  {
    psp_display_screen_menu();
    psp_sdl_back2_print(130, 110, "File loaded !",
                       PSP_MENU_NOTE_COLOR);
    psp_sdl_flip();
    sleep(1);
    return 1;
  ...
  {
    psp_display_screen_menu();
    psp_sdl_back2_print(130, 110, "Can't load file !",
...
psp_main_menu_load_slot()
{
  ...
  if (! HUGO.hugo_save_state[cur_slot].used) {
    ...
    psp_display_screen_menu();
    psp_sdl_back2_print(130, 110, "Slot is empty !",
                       PSP_MENU_NOTE_COLOR);
    psp_sdl_flip();
    sleep(1);
...
psp_main_menu_save_slot()
{
  ...
  {
    psp_display_screen_menu();
    psp_sdl_back2_print(130, 110, "File saved !",
                       PSP_MENU_NOTE_COLOR);
    psp_sdl_flip();
    sleep(1);
  }
  ...
  {
    psp_display_screen_menu();
    psp_sdl_back2_print(130, 110, "Can't save file !",
                       PSP_MENU_WARNING_COLOR);
    psp_sdl_flip();
    sleep(1);
  }
...
psp_main_menu_del_slot()
{
  ...
  {
    psp_display_screen_menu();
    psp_sdl_back2_print(130, 110, "File deleted !",
                       PSP_MENU_NOTE_COLOR);
    psp_sdl_flip();
    sleep(1);
  }
  ...
  {
    psp_display_screen_menu();
    psp_sdl_back2_print(130, 110, "Can't delete file !",
                       PSP_MENU_WARNING_COLOR);
    psp_sdl_flip();
    sleep(1);
  }
...
psp_main_menu_exit(void)
{
  ...
  psp_display_screen_menu();
  psp_sdl_back2_print(130, 110, "press B to confirm !", PSP_MENU_WARNING_COLOR);
  psp_sdl_flip();
