...
const char* DEFAULT_CDROM_PATH = "cdrom_path";
...
loc_hugo_save_paths()
{
  ...
  snprintf(FileName, MAX_PATH, "%s/set/default_paths.set", HUGO.hugo_home_dir);

  fp = fopen(FileName, "w");

  if (fp != (FILE *)0 ) {
      fprintf(fp, "%s=%s\n", DEFAULT_ROM_PATH, HUGO.hugo_rom_path);
      fprintf(fp, "%s=%s\n", DEFAULT_CDROM_PATH, HUGO.hugo_cdrom_path);
      fclose(fp);
...
loc_hugo_load_paths()
{
   ...
   snprintf(file_name, MAX_PATH, "%s/set/default_paths.set", HUGO.hugo_home_dir);
   ...
   while ((read = getline(&line, &len, fp)) != -1) {
      ...
      if(options){
          if(strcmp(options[0], DEFAULT_ROM_PATH) == 0){
            strcpy(HUGO.hugo_rom_path, options[1]);
          }else if(strcmp(options[0], DEFAULT_CDROM_PATH) == 0){
            strcpy(HUGO.hugo_cdrom_path, options[1]);
          }
      }
   }
...
hugo_load_rom(char *FileName, int zip_format)
{
  ...
  if (! error ) {
    ...
    strcpy(HUGO.hugo_rom_path, file_path);
    loc_hugo_save_paths();
  }
...
hugo_load_cd(char *FileName)
{
  ...
  if (! error ) {
    ...
    memcpy(file_path, FileName, strlen(FileName) - strlen(file_name));
    file_path[strlen(file_path) + 1] = '\0';
    strcpy(HUGO.hugo_cdrom_path, file_path);
    loc_hugo_save_paths();
  }
...
psp_global_initialize()
{
  ...
  strcpy(HUGO.hugo_home_dir,".");
   ...
   home_name = malloc(sizeof(char)*5 + 1);
   strcpy(home_name,".hugo");

   sprintf(HUGO.hugo_home_dir, "%s/%s", getenv("HOME"), home_name);
   mkdir(HUGO.hugo_home_dir, 0777);
   ...
   strcpy(HUGO.hugo_rom_path, tmp_directory);
   mkdir(tmp_directory, 0777);
   free (tmp_directory);

   tmp_directory = malloc(MAX_PATH + 1);
   sprintf(tmp_directory, "%s/%s/cd-roms/", getenv("HOME"), home_name);
   strcpy(HUGO.hugo_cdrom_path, tmp_directory);
   mkdir(tmp_directory, 0777);
   free (tmp_directory);
