...
# define MENU_SET_VOLUME        MENU_SET_SOUND+1
# define MENU_SET_FREQ          MENU_SET_VOLUME+1
# define MENU_SET_SPEED_LIMIT   MENU_SET_FREQ+1
# define MENU_SET_SKIP_FPS      MENU_SET_SPEED_LIMIT+1
# define MENU_SET_OVERCLOCK     MENU_SET_SKIP_FPS+1
# define MENU_SET_VIEW_FPS      MENU_SET_OVERCLOCK+1
# define MENU_SET_RENDER        MENU_SET_VIEW_FPS+1
# define MENU_SET_VSYNC         MENU_SET_RENDER+1
#if !defined(GCW0_MODE)
# define MENU_SET_CLOCK         MENU_SET_VSYNC+1
# define MENU_SET_LOAD          MENU_SET_CLOCK+1
#else
# define MENU_SET_LOAD          MENU_SET_VSYNC+1
#endif
...
psp_settings_menu_load(int format)
{
  ...
  if (ret ==  1) /* load OK */
  {
    ...
    psp_sdl_back2_print(180, 110, "File loaded !",
                       PSP_MENU_NOTE_COLOR);
    psp_sdl_flip();
    sleep(1);
  ...
  {
    psp_display_screen_settings_menu();
    psp_sdl_back2_print(180, 110, "Can't load file !",
...
psp_settings_menu_save()
{
  ...
  {
    psp_display_screen_settings_menu();
    psp_sdl_back2_print(180, 110, "File saved !",
                       PSP_MENU_NOTE_COLOR);
    psp_sdl_flip();
    sleep(1);
  }
  ...
  {
    psp_display_screen_settings_menu();
    psp_sdl_back2_print(180, 110, "Can't save file !",
                       PSP_MENU_WARNING_COLOR);
    psp_sdl_flip();
    sleep(1);
  }
...
{
  psp_display_screen_settings_menu();
  psp_sdl_back2_print(180,110, "Reset Settings !",
                     PSP_MENU_WARNING_COLOR);
  psp_sdl_flip();
