...
fill_cd_info ()
{
  ...
  switch (CD_emulation)
    {
    case 2:
      fseek(iso_FILE, 0L, SEEK_END);
      CD_track[0x02].length = ftell(iso_FILE) / 2048;
      fseek(iso_FILE, 0L, SEEK_SET);
      break;
    case 3:
  ...
  switch (CD_emulation)
    ...
    case 2:
      fseek(iso_FILE, 0L, SEEK_END);
      CD_track[nb_max_track].length = ftell(iso_FILE) / 2048;
      fseek(iso_FILE, 0L, SEEK_SET);
      break;
    case 3:
...
# endif
  ...
  if (((CD_emulation >= 2) && (CD_emulation <= 5)) && (!strcmp (ISO_filename, ""))){
    CD_emulation = 0;    // if no ISO name given, give up the emulation
  }
  ...
  if ((CD_emulation == 2) || (CD_emulation == 4)){
    ...
    errno = 0;
    iso_FILE = fopen (ISO_filename, "rb");
